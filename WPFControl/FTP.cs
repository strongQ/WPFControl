...
namespace UcMyVoice
{
    ...
    public class FTP
    {
        ...
        /* Download File */
        public async Task<string> downloadFull(string remoteFullFile, string localFile)
        {
            try
            {
                /* Create an FTP Request */
                 ftpRequest = (FtpWebRequest)FtpWebRequest.Create(remoteFullFile);
        ...
        {
            await downloadFull(host + "/" + remoteFile,localFile);
        }
        ...
        public void upload(string remoteFile, byte[] localFile)
        {
            try
            {
                /* Create an FTP Request */
                ...
                /* Establish Return Communication with the FTP Server */
                ftpStream = ftpRequest.GetRequestStream();
                MemoryStream ms = new MemoryStream(localFile);
                byte[] byteBuffer = new byte[bufferSize];
                int bytesSent = ms.Read(byteBuffer, 0, bufferSize);
                /* Upload the File by Sending the Buffered Data Until the Transfer is Complete */
                try
                {
                    while (bytesSent != 0)
                    {
                        ftpStream.Write(byteBuffer, 0, bytesSent);
                        bytesSent = ms.Read(byteBuffer, 0, bufferSize);
                    }
                }
                catch (Exception ex) { Console.WriteLine(ex.ToString()); }
                /* Resource Cleanup */
